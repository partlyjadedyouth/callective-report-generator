#!/usr/bin/env python3
"""
Script to fetch daily user counts from consolidated app analysis file.
...
def fetch_daily_user_counts():
    """
    Fetch daily user counts from consolidated app analysis file.
    ...
    try:
        ...
        if "daily_user_counts" in data:
            daily_user_counts = data["daily_user_counts"]
            print(f"Loaded {len(daily_user_counts)} daily counts from {file_path.name}")
...
def main():
    ...
    stats_text = f"Total: {sum(user_counts)} counts\nAverage: {round(sum(user_counts) / len(user_counts), 2)} users per day"
    plt.text(0.05, 1, stats_text)
