#!/usr/bin/env python3
"""
BAT Secondary Summary Script

This script extracts BAT_secondary scores from the analysis data and prints them in CSV format.
BAT_secondary scores are collected every 2 weeks (0주차, 2주차, 4주차, 6주차, 8주차, 10주차, 12주차).
"""
...
def extract_bat_secondary_scores(participant_data: Dict[str, Any], participant_id: str) -> list:
    """Extract BAT_secondary scores for a participant across all measurement weeks."""
    rows = []
    ...
    for week in target_weeks:
        ...
        # Get total BAT_secondary score from category_averages
        category_averages = week_data.get('category_averages', {})
        total_score = category_averages.get('BAT_secondary', '')
        
        # Get individual component scores from type_averages
        type_averages = week_data.get('type_averages', {})
        bat_secondary_data = type_averages.get('BAT_secondary', {})
        
        # Skip weeks without BAT_secondary data
        if not bat_secondary_data and total_score == '':
            continue
            
        time_symbol = week_to_time_symbol(week)
        
        # Extract the 2 BAT_secondary components
        physical = bat_secondary_data.get('신체적 호소', '')  # Physical complaints
        emotional = bat_secondary_data.get('심리적 호소', '')  # Emotional complaints
...
def print_csv_format(all_rows: list):
    ...
    # Print header
    headers = ['Participant', 'Week', 'Physical', 'Emotional', 'Total']
    print(','.join(headers))
...
def main():
    ...
    try:
        ...
        for participant in participants:
            ...
            # Extract BAT_secondary scores for this participant
            participant_rows = extract_bat_secondary_scores(participant, participant_id)
